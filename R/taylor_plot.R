...
taylor_plot <- function(min_R = 0.25, max_R = 1.75, contours = 7, n_lines = 10,x_0 = 1, ref_r_min=0.25, ref_r_max=2,ref_contours = 7) {
  ...
  taylor_plot <- ggplot() + ggforce::geom_arc(aes(x0=0, y0=0, r=r, start=0, end=pi/2),
                                     data=arcs)  +
    ...
    geom_text(data=seg_data,aes(x=1.05*x2,y=1.05*y2,label=round(rSeq,1)),size=3) +
    ggforce::geom_arc(aes(x0=x_0, y0=0, r=ref_r, start=angle_begin, end=angle_end),
             data=small_arcs,linetype=2) +
    geom_point(data=data.frame(x=x_0,y=0),aes(x=x,y=y),size=4,color='red',shape=15)
