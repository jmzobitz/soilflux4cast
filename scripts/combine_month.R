...
if (length(args) < 1) {
  stop("Usage: Rscript scripts/combine_month.R <YYYY-MM>")
}
curr_month <- args[1]
...
message("[combine_month] looking for files for month: ", curr_month)

# Find env files recursively (works whether artifacts were copied or left under site-*/)
env_files <- list.files(
  path = ".",
  pattern = paste0("soil_drivers-.*-", curr_month, "\\.csv$"),
  recursive = TRUE,
  full.names = TRUE
) %>% stringr::str_subset("soil_drivers-")

if (length(env_files) > 0) {
  message("[combine_month] found env files: ", paste(env_files, collapse = ", "))
  out_env <- tibble(
    site_id = str_extract(basename(env_files), "(?<=soil_drivers-)[A-Za-z0-9]{4}"),
    values = lapply(env_files, readr::read_csv)
  ) %>% unnest(values)
  
  dir.create(file.path("data", "drivers"), showWarnings = FALSE, recursive = TRUE)
...
flux_files <- list.files(
  path = ".",
  pattern = paste0("forecast_prediction-.*-", curr_month, "\\.csv$"),
  recursive = TRUE,
  full.names = TRUE
) %>% stringr::str_subset("forecast_prediction-")

if (length(flux_files) > 0) {
  message("[combine_month] found flux files: ", paste(flux_files, collapse = ", "))
  out_flux <- tibble(
